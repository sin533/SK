...
def dataset_mapping(x):
    return {
        "x": tokenizer.detokenize(tokenizer.tokenize(x["text"], pos_tagging=False)[:512]),#取前512个词
...
def main():
    ...
    dataset = datasets.load_dataset('csv', data_files='small-117M-k40.test.csv', split='train[:2]').map(
        function=dataset_mapping)
    print("Start attack")
    attack_eval = OpenAttack.AttackEval(attacker, victim)  # 开始进行攻击评估
