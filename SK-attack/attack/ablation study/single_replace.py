...
class SKAttacker(OpenAttack.attackers.ClassificationAttacker):  # base.py-classification.py
    ...
    def selectBUG(self, word, word_pos, stop, stop_orig, sen_idx, X, clsf, goal):
        ...
        candidate = bugs['replace']#获取所有候选句
...
def main():
    ...
    tokenizer1 = AutoTokenizer.from_pretrained("MohammadKarami/electra-human-detector")
    model1 = AutoModelForSequenceClassification.from_pretrained("MohammadKarami/electra-human-detector")
    # print(model)
    victim = OpenAttack.classifiers.TransformersClassifier(model1, tokenizer1, model1.electra.embeddings.word_embeddings)
    print('New Attacker:SKAttacker')
    attacker = SKAttacker()  # 设计攻击方式
    dataset = datasets.load_dataset('csv', data_files='small-117M-k40.test.csv', split='train[130:145]').map(
