...
def main():
    ...
    print('New Attacker:TextFoolerAttacker')
    attacker = OpenAttack.attackers.TextFoolerAttacker()  # 攻击模型
    dataset = datasets.load_dataset('csv',data_files = 'small-117M-k40.test.csv',split='train[:30]')
    dataset = dataset.map(function=data_mapping)
    print("Start attack")
    attack_eval = OpenAttack.AttackEval(attacker, victim,metrics=[SentenceSim(),BLEU(PunctTokenizer()),
                                                                                    Modification(PunctTokenizer()), GPT2LM()
    ])  # 评估攻击效果
